...
export default function AdminProductsPage() {
  ...
  const token = () => {
    ...
    if (!stored?.token) console.error("No auth token found");
    return stored?.token ?? "";
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res  = await fetch(url, { method: modal === "edit" ? "PUT" : "POST", headers: headers(), body: JSON.stringify(body) });
                    ...
                    <tr key={p.id}>
                      ...
                      <td>
                        <div style={{ display: "flex", gap: 6 }}>
                          ...
                          <button className="db-action-btn db-action-danger" onClick={() => handleDelete(p.id, p.name)} style={{ display: "flex", alignItems: "center", gap: 4 }}>
